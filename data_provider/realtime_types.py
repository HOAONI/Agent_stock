...
class CircuitBreaker:
    ...
    def record_failure(self, source: str, error: str | None = None) -> None:
        ...
        if state["state"] == self.HALF_OPEN:
            ...
            state["state"] = self.OPEN
            logger.warning(
                f"[熔断器] {source} 连续失败 {state['failures']} 次，进入熔断状态 " f"(冷却 {self.cooldown_seconds}s)"
            )
            if error:
...
_realtime_circuit_breaker = CircuitBreaker(
    failure_threshold=3, cooldown_seconds=300.0, half_open_max_calls=1  # 连续失败3次熔断  # 冷却5分钟
)

# 筹码接口熔断器（更保守的策略，因为该接口更不稳定）
_chip_circuit_breaker = CircuitBreaker(
    failure_threshold=2, cooldown_seconds=600.0, half_open_max_calls=1  # 连续失败2次熔断  # 冷却10分钟
)
