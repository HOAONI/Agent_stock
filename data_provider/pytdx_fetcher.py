...
class PytdxFetcher(BaseFetcher):
    ...
    @contextmanager
    def _pytdx_session(self) -> Generator[Any, None, None]:
        """
        Pytdx 连接上下文管理器
        ...
        使用示例：
            with self._pytdx_session() as api:
                # 在这里执行数据查询
        """
        TdxHq_API: Any | None = self._get_pytdx()
        ...
        with self._pytdx_session() as api:
            try:
                ...
                data = api.get_security_bars(
                    category=9, market=market, code=code, start=0, count=count  # 日线  # 从最新开始
                )
