# pyright: reportAssignmentType=false, reportArgumentType=false, reportAttributeAccessIssue=false, reportCallIssue=false, reportOptionalOperand=false, reportOperatorIssue=false
# -*- coding: utf-8 -*-
"""AkShare 主数据源实现。
...
class AkshareFetcher(BaseFetcher):
    ...
    def _get_stock_realtime_quote_sina(self, stock_code: str) -> UnifiedRealtimeQuote | None:
        ...
        try:
            ...
            # 新浪数据字段顺序：
            # 第 0 项为名称，第 1 项为今开，第 2 项为昨收，第 3 项为最新价，第 4 项为最高，
            # 第 5 项为最低，第 6 项为买一价，第 7 项为卖一价，第 8 项为成交量(股)，第 9 项为成交额(元)，第 30 项为日期，第 31 项为时间
            # 使用 realtime_types.py 中的统一转换函数
            ...
            logger.info(
                f"[实时行情-新浪] {stock_code} {quote.name}: 价格={quote.price}, " f"涨跌={quote.change_pct:.2f}%"
                if quote.change_pct
                else ""
            )
            return quote
    ...
    def _calc_market_stats(
        self, df: pd.DataFrame, change_col: str, amount_col: str | None = None
    ) -> dict[str, Any] | None:
