...
class NotificationService:
    ...
    def generate_agent_execution_report(self, run_payload: Dict[str, Any]) -> str:
        """Generate markdown report for one multi-agent execution cycle."""
        run_id = run_payload.get("run_id", "")
        mode = run_payload.get("mode", "once")
        trade_date = run_payload.get("trade_date", "")
        account = run_payload.get("account_snapshot", {}) or {}
        results = run_payload.get("results", []) or []

        lines = [
            f"# 🤖 Multi-Agent Execution Report ({trade_date})",
            "",
            f"- Run ID: `{run_id}`",
            f"- Mode: `{mode}`",
            f"- Cash: **{float(account.get('cash', 0.0)):.2f}**",
            f"- Market Value: **{float(account.get('total_market_value', 0.0)):.2f}**",
            f"- Total Asset: **{float(account.get('total_asset', 0.0)):.2f}**",
            "",
            "| Code | Advice | Target Weight | Action | Qty | Fill | Cash After | Position After |",
            "|---|---|---:|---|---:|---:|---:|---:|",
        ]
        ...
        for item in results:
            ...
            lines.append(
                "| {code} | {advice} | {weight:.4f} | {action} | {qty} | {fill} | {cash_after:.2f} | {pos_after} |".format(
                    code=item.get("code", ""),
                    advice=signal.get("operation_advice", "-"),
                    weight=float(risk.get("target_weight", 0.0) or 0.0),
                    action=execution.get("action", "none"),
                    qty=int(execution.get("traded_qty", 0) or 0),
                    fill=(
                        f"{float(execution['fill_price']):.4f}"
                        if execution.get("fill_price") is not None
                        else "-"
                    ),
                    cash_after=float(execution.get("cash_after", 0.0) or 0.0),
                    pos_after=int(execution.get("position_after", 0) or 0),
                )
            )
