...
def mask_secret(value: Optional[str]) -> str:
    """Mask sensitive secret-like values for logs/errors."""
    text = str(value or "")
    if not text:
        return ""
    if len(text) <= 6:
...
def redact_sensitive_payload(payload: Any) -> Any:
    """Redact sensitive keys recursively from dict/list payloads."""
    if payload is None:
        return None
...
class RuntimeAccountConfig:
    """Request-level account overrides."""
...
class RuntimeStrategyConfig:
    """Request-level strategy overrides."""
...
class RuntimeExecutionConfig:
    """Request-level execution overrides."""
...
class AgentRuntimeConfig:
    """Request-level runtime override bundle."""
...
class Config:
    ...
    database_url: Optional[str] = None  # Preferred runtime database URL (PostgreSQL in service mode)
    ...
    # === Agent paper-trading configuration ===
    agent_enabled: bool = False
    agent_run_mode: str = "once"  # once/realtime
    ...
    agent_ai_refresh_policy: str = "daily_once"  # daily_once/always
    ...
    agent_notify_on_no_trade: bool = False
    ...
    def _load_from_env(cls) -> 'Config':
        ...
        return cls(
            ...
            agent_run_mode=os.getenv('AGENT_RUN_MODE', 'once').lower(),
            ...
            agent_ai_refresh_policy=os.getenv('AGENT_AI_REFRESH_POLICY', 'daily_once').lower(),
            ...
            agent_notify_on_no_trade=os.getenv('AGENT_NOTIFY_ON_NO_TRADE', 'false').lower() == 'true',
            agent_service_mode=os.getenv('AGENT_SERVICE_MODE', 'false').lower() == 'true',
    ...
    def clone_with_overrides(self, **overrides: Any) -> 'Config':
        """
        Build an independent config object with request-level overrides.

        This helper never mutates the singleton instance and is safe for
        concurrent request-scoped usage.
        """
    ...
    def validate(self) -> List[str]:
        ...
        if self.agent_run_mode not in ("once", "realtime"):
            warnings.append("警告：AGENT_RUN_MODE 非法，建议使用 once 或 realtime")
        if self.agent_max_single_position_pct > 1.0:
            warnings.append("警告：AGENT_MAX_SINGLE_POSITION_PCT > 1.0，建议使用 0~1 区间")
        if self.agent_max_total_exposure_pct > 1.0:
            warnings.append("警告：AGENT_MAX_TOTAL_EXPOSURE_PCT > 1.0，建议使用 0~1 区间")
        if self.agent_service_mode and not self.database_url:
            warnings.append("警告：AGENT_SERVICE_MODE=true 时必须配置 DATABASE_URL")
        if self.agent_service_mode and not self.agent_service_auth_token:
            warnings.append("警告：AGENT_SERVICE_MODE=true 时必须配置 AGENT_SERVICE_AUTH_TOKEN")

        return warnings

    def validate_service_requirements(self) -> None:
        """Validate required settings for service mode."""
        missing = []
        ...
        if missing:
            joined = ", ".join(missing)
            raise ValueError(f"Missing required service configuration: {joined}")
